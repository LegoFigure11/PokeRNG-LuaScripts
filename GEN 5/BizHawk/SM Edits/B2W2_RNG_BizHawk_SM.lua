...
local JUMP_DATA = {
    { 0X6C078965, 0X269EC3 }, { 0X54341D9, 0X55AE9CB2 }, { 0X285E9F1, 0XC910A194 },
    { 0XAE3294E1, 0X4EAC71E8 }, { 0X5A78EDC1, 0X1566AED0 },
    { 0X75BEEB81, 0X592709A0 }, { 0X56221701, 0X6068C340 },
    { 0XCA552E01, 0X98DC4680 }, { 0X28EE5C01, 0X2EE38D00 },
    { 0X82ECB801, 0X3A731A00 }, { 0XCA197001, 0X27963400 },
    { 0XA532E001, 0X19EC6800 }, { 0X8E65C001, 0X5ED8D000 },
    { 0X2CCB8001, 0X69B1A000 }, { 0X99970001, 0X83634000 },
    { 0X332E0001, 0XC6C68000 }, { 0X665C0001, 0X8D8D0000 },
    { 0XCCB80001, 0X1B1A0000 }, { 0X99700001, 0X36340000 },
    { 0X32E00001, 0X6C680000 }, { 0X65C00001, 0XD8D00000 },
    { 0XCB800001, 0XB1A00000 }, { 0X97000001, 0X63400000 },
    { 0X2E000001, 0XC6800000 }, { 0X5C000001, 0X8D000000 },
    { 0XB8000001, 0X1A000000 }, { 0X70000001, 0X34000000 },
    { 0XE0000001, 0X68000000 }, { 0XC0000001, 0XD0000000 },
    { 0X80000001, 0XA0000000 }, { 0x1, 0X40000000 }, { 0x1, 0X80000000 }
}
...
local abilityNamesList = {
    ...
    "Reckless", "Multitype", "Flower Gift", "Bad Dreams", -- Gen 5
    "Pickpocket", "Sheer Force", "Contrary", "Unnerve", "Defiant", "Defeatist",
    "Cursed Body", "Healer", "Friend Guard", "Weak Armor", "Heavy Metal",
    "Light Metal", "Multiscale", "Toxic Boost", "Flare Boost", "Harvest",
    "Telepathy", "Moody", "Overcoat", "Poison Touch", "Regenerator",
    "Big Pecks", "Sand Rush", "Wonder Skin", "Analytic", "Illusion", "Imposter",
    "Infiltrator", "Mummy", "Moxie", "Justified", "Rattled", "Magic Bounce",
...
local itemNamesList = {
    ...
    "Loot Sack", "Rule Book", "Poké Radar", "Point Card", "Journal",
    ...
    "Plume Fossil", "Liberty Pass", "Pass Orb", "Dream Ball", "Poké Toy",
...
local mtSeedAddr, mtIndexAddr, currentSeedAddr, boxAddr, partySlotsCounterAddr,
partyAddr, trainerIDsAddr, cgearEnemyAddr, currBoxIndexAddr,
partySelectedSlotIndexAddr, partyStatsSelectedSlotIndexAddr, enemyAddr,
pokemonBoxStatsAddr, boxSelectedSlotIndexAddr
...
function printGameInfo()
    ...
    elseif gameVersion ~= "Black 2" and gameVersion ~= "White 2" then
        print(string.format(
            "Version: %s - Wrong game version! Use Black 2/White 2 instead\n",
            gameVersion))
...
local mode, index = {
    "None", "Capture", "Breeding", "C-Gear", "Pandora", "Pokemon Info"
}, 1
...
function setBackgroundBoxes() -- Set transparent black boxes
    ...
    if mode[index] == "Capture" or mode[index] == "Breeding" or mode[index] ==
        "C-Gear" or mode[index] == "Pokemon Info" then
...
local initialSeedFlag, prevMTSeed, initialSeedHigh, initialSeedLow,
tempCurrentSeedLow = false, 0, 0, 0, 0
...
function checkInitialSeedGeneration(mtSeed, currentHigh, currentLow)
    if currentLow ~= 0 and not initialSeedFlag then -- Set the initial seed when the LCRNG current seed address is initialized in RAM
        ...
        print(string.format("Initial Seed: %08X%08X", initialSeedHigh,
            initialSeedLow))
    end
...
function getRngInfo()
    ...
    prevMTSeed = mtSeed
    advances = mtSeed == currentHigh and 0 or advances +
        LCRNGDistance(tempCurrentSeedLow, currentLow)
...
function showRngInfo()
    ...
    if showRngInfoText and mode[index] ~= "None" then
        ...
        gui.text(5, 626, string.format("Initial Seed: %08X%08X",
            initialSeedHigh, initialSeedLow))
        gui.text(5, 644, string.format("Current Seed: %08X%08X",
            currentSeedHigh, currentSeedLow))
...
function getSlotInput()
    ...
    if (key["Number3"] or key["Keypad3"]) and
        (not prevKeySlot["Keypad3"] and not prevKeySlot["Keypad3"]) then
        leftSlotArrowColor = "orange"
        slotIndex = slotIndex - 1 < 0 and 2 or slotIndex - 1
    elseif (key["Number4"] or key["Keypad4"]) and
        (not prevKeySlot["Keypad4"] and not prevKeySlot["Keypad4"]) then
...
function getBits(a, b, d) return (a >> b) % (1 << d) end
...
function getHPTypeAndPower(hpIV, atkIV, defIV, spAtkIV, spDefIV, spdIV)
    ...
    local hpPower = 30 +
        floor(
            (((hpIV >> 1) % 2) + 2 * ((atkIV >> 1) % 2) + 4 *
                ((defIV >> 1) % 2) + 8 * ((spdIV >> 1) % 2) + 16 *
                ((spAtkIV >> 1) % 2) + 32 * ((spDefIV >> 1) % 2)) *
            40 / 63)
...
function showMoves(moveIndexesList)
    gui.text(5, 551, "Move: " ..
        moveNamesList[moveIndexesList[1] > 560 and 1 or
        moveIndexesList[1]])
    gui.text(5, 569, "Move: " ..
        moveNamesList[moveIndexesList[2] > 560 and 1 or
        moveIndexesList[2]])
    gui.text(5, 587, "Move: " ..
        moveNamesList[moveIndexesList[3] > 560 and 1 or
        moveIndexesList[3]])
    gui.text(5, 605, "Move: " ..
        moveNamesList[moveIndexesList[4] > 560 and 1 or
        moveIndexesList[4]])
end
...
function showInfo(pidAddr)
    ...
    local speciesDexIndex = read16Bit(pidAddr + growthOffset + 0x8) ~
        (prng >> 16)

    prng = LCRNG(prng, 0x41C64E6D, 0x6073)
    local heldItemIndex = (read16Bit(pidAddr + growthOffset + 0xA) ~
        (prng >> 16)) + 1
    ...
    prng = LCRNG(prng, 0x41C64E6D, 0x6073)
    local hiddenAbilityFlag = (read16Bit(pidAddr + attacksOffset + 0x22) ~
        (prng >> 16)) & 1 == 1

    if mode[index] ~= "Breeding" or isEgg then
        gui.text(5, 404, "Species: " ..
            speciesNamesList[(speciesDexIndex > 649 or speciesDexIndex <
                1) and 1 or speciesDexIndex])
        gui.text(5, 422, "PID:")
        gui.text(53, 422, string.format("%08X%s", pokemonPID, shinyType),
            shinyTypeTextColor)
        gui.text(5, 440, "Nature: " ..
            natureNamesList[(natureIndex > 25 or natureIndex == nil) and
            1 or natureIndex])
        gui.text(5, 458,
            string.format("Ability: %s (%s)",
                abilityNamesList[(abilityIndex > 164 or
                    abilityIndex < 1) and 1 or abilityIndex],
                hiddenAbilityFlag and "H" or abilityIndex ==
                pokemonAbilities[(speciesDexIndex > 649 or
                    speciesDexIndex < 1) and 1 or
                speciesDexIndex][1] and "0" or "1"))

        showIVsAndHP(ivsValue)

        gui.text(5, 512, "Held item: " ..
            itemNamesList[(heldItemIndex > 639) and 1 or heldItemIndex])
...
function showPokemonInfo()
    ...
    elseif infoMode[infoIndex] == "Party Stats" then
        ...
        local pokemonPartyStatsAddr = partyAddr +
            (partyStatsSelectedSlotIndex * 0xDC)
