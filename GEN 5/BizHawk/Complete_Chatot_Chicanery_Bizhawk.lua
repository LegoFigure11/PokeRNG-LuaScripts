...
while true do
	...
	key = input.get()
	if key["Number0"] or key["Keypad0"] then runbtn = 1 end
	if key["Number9"] or key["Keypad9"] then flipbtn = -1 * flipbtn end

	if runbtn == 0 then
		gui.text(1, (384 + flipbtn * 320),
			"Press 0 to run, advances set: " .. maxAdvances, "cyan")
	...
	elseif runbtn == 1 then
		if pidAdvances <= maxAdvances then
			...
			gui.text(1, (384 + flipbtn * 320), "Advances: " .. pidAdvances - 1,
				"cyan")
		else
			gui.text(1, (384 + flipbtn * 320),
				"Advances: " .. pidAdvances - 1 .. ", DONE!", "cyan")
		end
	end
