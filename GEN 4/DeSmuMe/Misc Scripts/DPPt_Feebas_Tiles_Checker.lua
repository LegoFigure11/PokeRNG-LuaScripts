...
function findFeebasTiles(seed)
    for i = 0, 3 do
        feebasTiles[i + 1] = (band(rshift(seed, (24 - (8 * i))), 0xFF) % 0x84) +
            (0x84 * i)
    end
end
...
function showWaterTiles()
    ...
    if posY >= 17 and posY <= 51 then
        ...
        if upTile then
            gui.text(121, -120, string.format("%2d", upTile),
                getTileColor(upTile, ""))
        end

        if downTile then
            gui.text(121, -86, string.format("%2d", downTile),
                getTileColor(downTile, ""))
        end

        if currTile then
            gui.text(121, -102, string.format("%2d", currTile),
                getTileColor(currTile, "currTile"))
        end

        if leftTile then
            gui.text(100, -102, string.format("%2d", leftTile),
                getTileColor(leftTile, ""))
        end

        if rightTile then
            gui.text(142, -102, string.format("%2d", rightTile),
                getTileColor(rightTile, ""))
        end
    end
end
...
function main()
    ...
    if isMtCoronetFeebasRoom() then showWaterTiles() end
