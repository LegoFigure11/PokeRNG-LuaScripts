...
function parsePKM(address)
    ...
    print(string.format("Species: %3d\tNature:\t", memory.readbyte(address)) ..
    naturestr[exp % 25 + 1] .. "\t" .. shiny(atkdef, spespc))
    ...
    print(string.format("EVs:\t%d\t%d\t%d\t%d\t%d", Readbytes(address + 0xB, 2), Readbytes(address + 0xD, 2),
        Readbytes(address + 0xF, 2), Readbytes(address + 0x11, 2), Readbytes(address + 0x13, 2)))
end
...
function ShowPKM(address)
    ...
    local hp = atk % 2 * 8 + def % 2 * 4 + spe % 2 * 2 + spc % 2
    gui.text(2, 0,
        string.format("Species: %3d\tNature:\t", memory.readbyte(address)) ..
        naturestr[exp % 25 + 1] .. "\t" .. shiny(atkdef, spespc), defaultfont)
    ...
    gui.text(2, 40,
        string.format("EVs:%6d%6d%6d%6d%6d", Readbytes(address + 0xB, 2), Readbytes(address + 0xD, 2),
            Readbytes(address + 0xF, 2), Readbytes(address + 0x11, 2), Readbytes(address + 0x13, 2)), defaultfont)
end
