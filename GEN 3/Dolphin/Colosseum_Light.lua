...
function setInfo(enemyAddr, partyAddr, boxAddr)
    ...
    while enemyShadowID == 0 and i < 6 do
        ...
        catchRateValue = calcCatchRate(read16Bit(shadowEnemyAddr + 0x86), 1,
        ...
        enemyHPType = ((enemyHpIV % 2 + 2 * (enemyAtkIV % 2) + 4 *
                (enemyDefIV % 2) + 8 * (enemySpdIV % 2) + 16 *
                (enemySpAtkIV % 2) + 32 * (enemySpDefIV % 2)) * 15) //
            63
        enemyHPPower = (((enemyHpIV & 2) / 2 + (enemyAtkIV & 2) + 2 *
    ...
    local speciesText = string.format("\n\nOpponent\n\nSpecies: %s",
        enemySpeciesName)
    local PIDsText = string.format("\nPID: %08X %s", enemyPID, shinyCheck(
        enemyPID >> 16, enemyPID & 0xFFFF,
        enemyOTID, enemyOTSID))
    local naturesText = string.format("\nNature: %s", enemyNatureName)
    local ivsText = string.format("\nIVs: %02d/%02d/%02d/%02d/%02d/%02d",
        enemyHpIV, enemyAtkIV, enemyDefIV,
        enemySpAtkIV, enemySpDefIV, enemySpdIV)
    local HPText = string.format("\nHPower: %s %02d",
        HPTypeNamesList[enemyHPType + 1], enemyHPPower)
    local catchRngText = string.format("\nCatch Rate Value: %.0f",
        catchRateValue)
    local text = speciesText .. PIDsText .. naturesText .. ivsText .. HPText ..
        catchRngText
    ...
    PIDsText = string.format("\nPID: %08X %s", partyPID, shinyCheck(
        partyPID >> 16, partyPID & 0xFFFF, partyOTID,
        partyOTSID))
    naturesText = string.format("\nNature: %s", partyNatureName)
    ivsText = string.format("\nIVs: %02d/%02d/%02d/%02d/%02d/%02d", partyHpIV,
        partyAtkIV, partyDefIV, partySpAtkIV, partySpDefIV,
        partySpdIV)
    HPText = string.format("\nHPower: %s %02d",
        HPTypeNamesList[partyHPType + 1], partyHPPower)
    local partyInfoText = speciesText .. PIDsText .. naturesText .. ivsText ..
        HPText
    ...
    local boxHPType = ((boxHpIV % 2 + 2 * (boxAtkIV % 2) + 4 * (boxDefIV % 2) +
        8 * (boxSpdIV % 2) + 16 * (boxSpAtkIV % 2) + 32 *
        (boxSpDefIV % 2)) * 15) // 63
    local boxHPPower =
        (((boxHpIV & 2) / 2 + (boxAtkIV & 2) + 2 * (boxDefIV & 2) + 4 *
            (boxSpdIV & 2) + 8 * (boxSpAtkIV & 2) + 16 * (boxSpDefIV & 2)) * 40) //
        63 + 30

    speciesText = string.format("\nBox\n\nSpecies: %s", boxSpeciesName)
    PIDsText = string.format("\nPID: %08X %s", boxPID, shinyCheck(boxPID >> 16,
        boxPID &
        0xFFFF,
        boxOTID,
        boxOTSID))
    naturesText = string.format("\nNature: %s", boxNatureName)
    ivsText = string.format("\nIVs: %02d/%02d/%02d/%02d/%02d/%02d", boxHpIV,
        boxAtkIV, boxDefIV, boxSpAtkIV, boxSpDefIV, boxSpdIV)
    HPText = string.format("\nHPower: %s %02d", HPTypeNamesList[boxHPType + 1],
        boxHPPower)
    local boxInfoText = speciesText .. PIDsText .. naturesText .. ivsText ..
        HPText
