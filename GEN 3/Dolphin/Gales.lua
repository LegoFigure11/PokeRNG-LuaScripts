...
local catchRatesList = {
    ...
    45, 255, 120, 45, 45, 235, 120, 75, 255, 90, 45, 45, 30, 70, 90, 225, 45,
    60, 190, 75, 190, 60, 25, 190, 75, 45, 25, 190, 45, 60, 120, 60, 190, 120,
    225, 75, 60, 190, 75, 45, 90, 15, 225, 45, 45, 120, 60, 45, 45, 45, 75, 45,
    45, 45, 45, 45, 30, 15, 15, 15, 45, 45, 10, 3, 3, 45, -- Gen3
    45, 45, 45, 45, 45, 45, 45, 45, 45, 255, 127, 255, 90, 255, 120, 45, 120,
    45, 255, 120, 45, 255, 120, 45, 200, 90, 255, 45, 235, 120, 45, 200, 75,
    255, 90, 255, 120, 45, 255, 120, 45, 190, 120, 45, 255, 200, 150, 255, 255,
    120, 90, 120, 180, 90, 45, 180, 90, 120, 80, 200, 200, 150, 150, 150, 225,
    75, 225, 60, 125, 60, 255, 150, 90, 255, 60, 255, 255, 120, 45, 190, 60,
    255, 80, 90, 90, 100, 90, 190, 75, 205, 155, 255, 90, 45, 45, 45, 45, 255,
    60, 45, 200, 225, 90, 190, 90, 45, 45, 30, 125, 190, 75, 255, 120, 45, 255,
    60, 60, 25, 225, 45, 45, 80, 3, 3, 15, 3, 3, 3, 3, 3, 5, 5, 3, 3, 3
}
...
function isBoxOpened() return read16Bit(boxFlagAddr) == 0x391 end
...
function setBoxAddr(partyAddr)
    ...
    if isBoxOpened() then
        return boxAddr +
            (0xC4 * (boxSelectedSlotNumber - boxSelectedSlotNumberOff)) +
            (0x14 * currBoxNumber) + (0xC4 * currBoxNumber * 0x1E)
    else
        return boxAddr
    end
end
...
function setPadding(maxLength, goodSpacing, stringVar)
    ...
    if stringVarLength <= maxLength then
        ...
        for i = 0, padding + goodSpacing do spaces = spaces .. " " end
    end
...
function setInfo(enemyAddr, partyAddr, boxAddr, i)
    ...
    if enemySpeciesDexIndex > 411 then enemySpeciesDexIndex = 0 end
    ...
    local catchRateValue = calcCatchRate(read16Bit(enemyAddr + 0x69), 1,
        catchRatesList[enemySpeciesDexNumber])
    ...
    local enemyHPType = ((enemyHpIV % 2 + 2 * (enemyAtkIV % 2) + 4 *
            (enemyDefIV % 2) + 8 * (enemySpdIV % 2) + 16 *
            (enemySpAtkIV % 2) + 32 * (enemySpDefIV % 2)) * 15) //
        63
    local enemyHPPower = (((enemyHpIV & 2) / 2 + (enemyAtkIV & 2) + 2 *
            (enemyDefIV & 2) + 4 * (enemySpdIV & 2) + 8 *
            (enemySpAtkIV & 2) + 16 * (enemySpDefIV & 2)) * 40) //
        63 + 30
    ...
    local partyHPType = ((partyHpIV % 2 + 2 * (partyAtkIV % 2) + 4 *
            (partyDefIV % 2) + 8 * (partySpdIV % 2) + 16 *
            (partySpAtkIV % 2) + 32 * (partySpDefIV % 2)) * 15) //
        63
    local partyHPPower = (((partyHpIV & 2) / 2 + (partyAtkIV & 2) + 2 *
            (partyDefIV & 2) + 4 * (partySpdIV & 2) + 8 *
            (partySpAtkIV & 2) + 16 * (partySpDefIV & 2)) * 40) //
        63 + 30
    ...
    if i == 0 then
        ...
        local boxHPType = ((boxHpIV % 2 + 2 * (boxAtkIV % 2) + 4 *
                (boxDefIV % 2) + 8 * (boxSpdIV % 2) + 16 *
                (boxSpAtkIV % 2) + 32 * (boxSpDefIV % 2)) * 15) //
            63
        local boxHPPower = (((boxHpIV & 2) / 2 + (boxAtkIV & 2) + 2 *
                (boxDefIV & 2) + 4 * (boxSpdIV & 2) + 8 *
                (boxSpAtkIV & 2) + 16 * (boxSpDefIV & 2)) * 40) //
            63 + 30

        speciesText = string.format("Species: %sSpecies: %sSpecies: %s",
            enemySpeciesName, partySpeciesName,
            boxSpeciesName)
        PIDsText = string.format("\nPID: %08X %s   PID: %08X %s   PID: %08X %s",
            enemyPID, shinyCheck(enemyPID >> 16,
                enemyPID & 0xFFFF,
                enemyOTID, enemyOTSID),
            partyPID, shinyCheck(partyPID >> 16,
                partyPID & 0xFFFF,
                partyOTID, partyOTSID),
            boxPID, shinyCheck(boxPID >> 16,
                boxPID & 0xFFFF, boxOTID,
                boxOTSID))
        naturesText = string.format("\nNature: %sNature: %sNature: %s",
            enemyNatureName, partyNatureName,
            boxNatureName)
        ivsText = string.format(
            "\nIVs: %02d/%02d/%02d/%02d/%02d/%02d   IVs: %02d/%02d/%02d/%02d/%02d/%02d   IVs: %02d/%02d/%02d/%02d/%02d/%02d",
            enemyHpIV, enemyAtkIV, enemyDefIV, enemySpAtkIV,
            enemySpDefIV, enemySpdIV, partyHpIV, partyAtkIV,
            partyDefIV, partySpAtkIV, partySpDefIV, partySpdIV,
            boxHpIV, boxAtkIV, boxDefIV, boxSpAtkIV, boxSpDefIV,
            boxSpdIV)
        ...
        HPText = string.format("\nHPower: %s %02d", HPTypeName, enemyHPPower) ..
            setPadding(11, 5, string.format("%s %02d", HPTypeName,
                enemyHPPower)) ..
            string.format("HPower: %s %02d", partyHPTypeName,
                partyHPPower) ..
            setPadding(11, 5, string.format("%s %02d", partyHPTypeName,
                partyHPPower)) ..
            string.format("HPower: %s %02d", boxHPTypeName, boxHPPower)
        catchRngText = string.format("\nCatch Rate Value: %.0f", catchRateValue)
        text = speciesText .. PIDsText .. naturesText .. ivsText .. HPText ..
            catchRngText
    else
        speciesText = string.format("Species: %sSpecies: %s", enemySpeciesName,
            partySpeciesName)
        PIDsText = string.format("\nPID: %08X %s   PID: %08X %s", enemyPID,
            shinyCheck(enemyPID >> 16, enemyPID & 0xFFFF,
                enemyOTID, enemyOTSID), partyPID,
            shinyCheck(partyPID >> 16, partyPID & 0xFFFF,
                partyOTID, partyOTSID))
        naturesText = string.format("\nNature: %sNature: %s", enemyNatureName,
            partyNatureName)
        ivsText = string.format(
...
function onScriptUpdate()
    ...
    if pointer ~= 0 then
        ...
        for i = 0, 5 do
            infoText = infoText ..
                setInfo(enemyAddr + (0xC4 * i),
                    partyAddr + (0xC4 * i), currBoxAddr, i) ..
                "\n\n"
        end
    end

    RNGInfoText = string.format(
        "Initial Seed: %08X\nCurrent Seed: %08X\nAdvances: %d",
        initSeed, currSeed, advances)
    IDsInfoText = string.format("\n\n\nTID: %05d\nSID: %05d", TID, SID)
    finalText = RNGInfoText .. infoText .. IDsInfoText
    SetScreenText(finalText)
end

function onScriptCancel() SetScreenText("") end
