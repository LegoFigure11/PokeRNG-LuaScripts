...
local locationNamesList = {
  ...
  "Route 131", "Route 132", "Route 133", "Route 134", "Underwater 1",
  "Underwater 2", "Underwater 3", "Underwater 4"
}
...
local emuWindow = {}
local mode = {
  "None", "Capture", "100% Catch", "Breeding", "Pandora", "Pokemon Info"
}
local index = 1
local prevKey = {}
...
function calcAdvancesJump(seed)
  ...
  if tempCurr ~= seed then
    ...
    while tempCurr ~= seed and tempCurr2 ~= seed and calibrationAdvances <
      99999 do
...
function showRngInfo(current, advances)
  if initSeed ~= nil then
    gui.text(emuWindow.leftPadding, emuWindow.bottomPadding - 36,
      string.format("Initial Seed: %04X", initSeed))
  end
...
function isRoamerAtPlayerLocation(mapGroupNum1, mapGroupNum2)
  mapGroupNum1 = lshift(band(mapGroupNum1, 0xFF) - 1, 8) +
      rshift(mapGroupNum1, 8)
...
function getBonusBall(speciesDexNumber, isSafariZone)
  ...
  local dexCaughtFlag = band(read8Bit(dexCaughtFlagAddr +
      band(
        rshift(
...
function showPokemonInfo()
  ...
  elseif infoMode[infoIndex] == "Party" then
    partySlotsCounter = read8Bit(partySlotsCounterAddr) - 1
    partySelectedSlotNumber = read8Bit(
      partySelectedSlotNumberAddr +
      (partySlotsCounter * 0x88))
...
while warning == "" do
  ...
  if mode[index] == "Capture" or mode[index] == "Breeding" or mode[index] ==
      "Pandora" or mode[index] == "Pokemon Info" then
    if showRngInfoText then showRngInfo(currSeed, advances) end
