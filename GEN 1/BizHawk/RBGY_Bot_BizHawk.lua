...
function showPartyPokemonInfo()
       ...
       for i = 0, partySlotsCounter do
              ...
              local atkDV, defDV, speDV, spcDV = getDVs(pokemonDVsAddr)
              local pokemonEXP = (0x10000 * read8Bit(pokemonEXPAddr)) + (0x100 * read8Bit(pokemonEXPAddr + 0x1)) +
              read8Bit(pokemonEXPAddr + 0x2)
              local pokemonNatureName = natureNamesList[(pokemonEXP % 25) + 1]

              if pokemonSpeciesName ~= nil then
                     gui.text(emuWindow.leftPadding + 1, (i + 3) * 18,
                            tostring(i + 1) ..
                            " " .. pokemonSpeciesName .. "\t" .. pokemonNatureName ..
                            shinyText(atkDV, defDV, speDV, spcDV))
              end
       end
end
